...
# DOCTORS LOCATION DATA (USING OSMNX)

# extract doctors locations within the greater manchester buffer polygon
...
# create doctors points (sometimes can be polygons points so convesrt it to their centroid if a polygon)
doctors_points = doctors.geometry.apply(lambda g: g.centroid).to_list()
...
# create a spatial index for doctors
doctors_idx = STRtree(doctors_points)
...
for id, pop in worst_pop_points.iterrows():
    ...
    # find the nearest doctors to each population point
    try:
        nearest_doctors_id = doctors_idx.nearest([pop_point])[0]
    except TypeError:
        nearest_doctors_id = doctors_idx.nearest(pop_point)
        
    nearest_doctors = doctors_points[nearest_doctors_id]
    
    # find the nearest node to the nearest doctors
    to_node = osm.distance.nearest_nodes(driving_graph, nearest_doctors.x, nearest_doctors.y)
...
worst_pop_points["doctors_driving_astar"] = travel_time_list
...
if len(valid_times) > 0:
    ...
    print(f"Minimum travel time to a doctors from TRSE vulnerable areas: {min(travel_time_list):,.0f} mins.")
    print(f"Mean travel time to a doctors from TRSE vulnerable areas: {mean_time:,.0f} mins.")
    print(f"Maximum travel time to a doctors from TRSE vulnerable areas: {max(travel_time_list):,.0f} mins.")
...
results = worst_pop_points[["OA21CD", "doctors_driving_astar"]]
...
doctors_points_plot = GeoDataFrame(geometry = doctors_points, crs = 4326).to_crs(27700)
...
gm_oas_plot.plot(
    ...
    legend_kwds = {
    'loc': 'lower right',
    'title': 'Shortest Travel Time to Nearest Doctors (mins)',
    'frameon': True,
    'borderpad': 0.6,
    'labelspacing': 0.5
    })
...
legend.set_title("Driving Travel Time to Nearest Doctors (minutes)", prop={'size':10})
for text in legend.get_texts():
    text.set_fontsize(9)

# plot the locations, coloured by distance to doctors
#doctors_points_plot.plot(
    #ax = my_ax,
    #linewidth = 0,
    #marker = 'x',
	#markersize = 100,
