...
# create points/geom for doctors (doctors can be polygons and thus the centroid needs to be found to have a single point)
doctors_points = [geom.centroid for geom in doctors.geometry.to_list()]
...
for id, pop in worst_pop_points.iterrows():
    ...
    # find the nearest doctor to each population point
    ...
    to_node_id = node_idx.nearest([nearest_doctors])[0]
...
print(f"Minimum distance to a doctors from TRSE vulnerable areas: {min(distances):,.0f}m.")
print(f"Mean distance to a doctors from TRSE vulnerable areas: {mean:,.0f}m.")
print(f"Maximum distance to a doctors from TRSE vulnerable areas: {max(distances):,.0f}m.")
...
doctors_points_plot = doctors.to_crs(27700)

doctors_points_plot = doctors_points_plot.geometry.centroid
...
# add title
title("Distance to Nearest Doctors in Greater Manchester for TRSE Vulnerable Individuals.")
...
worst_pop_points_plot.plot(
    ...
    legend_kwds = {
        'loc': 'lower right',
        'title': 'Distance to Nearest Doctors'
        }
    )
