...
# create primary school points (sometimes can be polygons points so convesrt it to their centroid if a polygon)
...
for id, pop in worst_pop_points.iterrows():
    ...
    # find the nearest node to the nearest primary school
    to_node = osm.distance.nearest_nodes(walking_graph, nearest_primary.x, nearest_primary.y)
...
worst_pop_points["primary_walking_astar"] = travel_time_list
...
results = worst_pop_points[["OA21CD", "primary_walking_astar"]]
...
gm_oas_plot.plot(
    ...
    legend_kwds = {
    'loc': 'lower right',
    'title': 'Shortest Walking Time to Nearest Primary School (mins)',
    'frameon': True,
    'borderpad': 0.6,
    'labelspacing': 0.5
    })
...
legend.set_title("Walking Travel Time to Nearest Primary School (minutes)", prop={'size':10})
