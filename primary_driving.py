...
# PRIMARY SCHOOLS DRIVING NETWORK ANALYSIS
...
def time_heuristic(node_a, node_b, max_speed_kph = 110):
    ...
    return distance_in_metres / max_speed_mps
...
# create primary points (sometimes can be polygons points so convesrt it to their centroid if a polygon)
primary_points = primary_schools.geometry.apply(lambda g: g.centroid).to_list()
...
# SPATIAL INDEX (PRIMARY SCHOOLSS)

# create a spatial index for primarys
primary_idx = STRtree(primary_points)
...
for id, pop in worst_pop_points.iterrows():
    ...
    from_node = osm.distance.nearest_nodes(driving_graph, pop_point.x, pop_point.y)
    
    # find the nearest primary to each population point
    try:
        nearest_primary_id = primary_idx.nearest([pop_point])[0]
    except TypeError:
        nearest_primary_id = primary_idx.nearest(pop_point)
        
    nearest_primary = primary_points[nearest_primary_id]
    
    # find the nearest node to the nearest primary
    to_node = osm.distance.nearest_nodes(driving_graph, nearest_primary.x, nearest_primary.y)
    ...
    try:
       	...
       	shortest_path = astar_path(driving_graph, from_node, to_node, time_heuristic, weight = "travel_time")
...
worst_pop_points["primary_driving_astar"] = travel_time_list
...
if len(valid_times) > 0:
    ...
    print(f"Minimum travel time to a primary school from TRSE vulnerable areas: {min(travel_time_list):,.0f} mins.")
    print(f"Mean travel time to a primary school from TRSE vulnerable areas: {mean_time:,.0f} mins.")
    print(f"Maximum travel time to a primary school from TRSE vulnerable areas: {max(travel_time_list):,.0f} mins.")
...
results = worst_pop_points[["OA21CD", "primary_driving_astar"]]
...
primary_points_plot = GeoDataFrame(geometry = primary_points, crs = 4326).to_crs(27700)
...
gm_oas_plot.plot(
    ...
    legend_kwds = {
    'loc': 'lower right',
    'title': 'Shortest Travel Time to Nearest Primary School (mins)',
    'frameon': True,
    'borderpad': 0.6,
    'labelspacing': 0.5
    })
...
labels[-2] = "10+ mins (High Spatial Barrier)"
labels[-3] = "5-10 mins (Moderate Spatial Barrier)"
labels[-4] = "0-5 mins (Low Spatial Barrier)"

for text, new_label in zip(legend.get_texts(), labels):
    text.set_text(new_label)
 
legend.set_title("Driving Travel Time to Nearest Primary School (minutes)", prop={'size':10})
for text in legend.get_texts():
    text.set_fontsize(9)

# plot the locations, coloured by distance to primarys
#primary_points_plot.plot(
    #ax = my_ax,
    #linewidth = 0,
    #marker = 'x',
	#markersize = 100,
