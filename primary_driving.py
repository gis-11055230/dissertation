...
# SPATIAL INDEX (PRIMARY SCHOOLS)

# create points/geom for primary schools (primary schools can be polygons and thus the centroid needs to be found to have a single point)
primary_schools_points = [geom.centroid for geom in primary_schools.geometry.to_list()]

# create a spatial index for primary schools
primary_schools_idx = STRtree(primary_schools_points)
...
for id, pop in worst_pop_points.iterrows():
    ...
    # find the nearest primary schools to each population point
    nearest_primary_schools_id = primary_schools_idx.nearest([pop_point])[0]
    nearest_primary_schools = primary_schools_points[nearest_primary_schools_id]
    
    # find the nearest node to the nearest primary schools
    to_node_id = node_idx.nearest([nearest_primary_schools])[0]
...
worst_pop_points["primary_schools_driving_astar"] = distances
...
print(f"Minimum distance to a primary school from TRSE vulnerable areas: {min(distances):,.0f}m.")
print(f"Mean distance to a primary school from TRSE vulnerable areas: {mean:,.0f}m.")
print(f"Maximum distance to a primary school from TRSE vulnerable areas: {max(distances):,.0f}m.")
...
primary_schools_points_plot = primary_schools.to_crs(27700)

primary_schools_points_plot = primary_schools_points_plot.geometry.centroid
...
# add title
title("Distance to Nearest Primary School in Greater Manchester for TRSE Vulnerable Individuals.")
...
# plot the locations, coloured by distance to Primary School
worst_pop_points_plot.plot(
    ax = my_ax,
    column = 'primary_schools_driving_astar',
    linewidth = 0,
	markersize = 100,
    cmap = 'viridis',
    scheme = 'quantiles',
    ...
    legend_kwds = {
        'loc': 'lower right',
        'title': 'Distance to Nearest Primary School'
        }
    )

# plot the locations, coloured by distance to Primary Schools
primary_schools_points_plot.plot(
    ax = my_ax,
    linewidth = 0,
    marker = 'x',
	markersize = 50,
