...
# JOB CENTRES DRIVING NETWORK ANALYSIS
...
for id, pop in worst_pop_points.iterrows():
    ...
    to_node = osm.distance.nearest_nodes(driving_graph, nearest_job_centres.x, nearest_job_centres.y)
...
worst_pop_points["job_centres_driving_astar"] = travel_time_list
...
if len(valid_times) > 0:
    ...
    print(f"Minimum travel time to a job centre from TRSE vulnerable areas: {min(valid_times):,.0f} mins.")
    print(f"Mean travel time to a job centre from TRSE vulnerable areas: {mean_time:,.0f} mins.")
    print(f"Maximum travel time to a job centre from TRSE vulnerable areas: {max(valid_times):,.0f} mins.")
...
results = worst_pop_points[["OA21CD", "job_centres_driving_astar"]]
...
gm_oas_plot.plot(
    ax = my_ax,						
    column = 'job_centres_driving_astar',
    cmap = 'YlOrRd',          
    scheme = 'user_defined',
...
legend.set_title("Driving Travel Time to Nearest Job Centre (minutes)", prop={'size':10})
