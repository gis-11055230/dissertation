# PRIMARY SCHOOLS PT NETWORK ANALYSIS
...
# PT TRAVEL TIME (FASTEST TO NEAREST PRIMARY SCHOOL)
...
# create destinations from primary_points (must be EPSG:4326) to use in the network
destinations = GeoDataFrame(
    {"id": list(range(len(primary_points)))},
    geometry = primary_points,
    crs = 4326)
...
# get the fastest primary for each OA centroid
nearest_pt = (
    travel_matrix
    .groupby("from_id", as_index=False)["travel_time"]
    .min()
    .rename(columns={"from_id": "OA21CD", "travel_time": "primary_pt_8am"}))

# join back to worst_pop_points
worst_pop_points = worst_pop_points.merge(nearest_pt, on="OA21CD", how="left")

# quick success stats
valid_times = worst_pop_points["primary_pt_8am"].dropna().to_list()
...
results = worst_pop_points[["OA21CD", "primary_pt_8am"]]
...
legend.set_title("Public Transport Travel Time to Nearest Primary School (minutes)\n(Departure Date: Wednesday 08:00)", prop={'size':10})
