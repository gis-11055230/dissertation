...
# PT TRAVEL TIME (FASTEST TO NEAREST DOCTORS)
...
# create destinations from doctors_points (must be EPSG:4326) to use in the network
destinations = GeoDataFrame(
    {"id": list(range(len(doctors_points)))},
    geometry = doctors_points,
    crs = 4326)
...
# get the fastest doctors for each OA centroid
nearest_pt = (
    travel_matrix
    .groupby("from_id", as_index=False)["travel_time"]
    .min()
    .rename(columns={"from_id": "OA21CD", "travel_time": "doctors_pt_8am"}))

# join back to worst_pop_points
worst_pop_points = worst_pop_points.merge(nearest_pt, on="OA21CD", how="left")

# quick success stats
valid_times = worst_pop_points["doctors_pt_8am"].dropna().to_list()
...
results = worst_pop_points[["OA21CD", "doctors_pt_8am"]]
...
legend.set_title("Public Transport Travel Time to Nearest Doctors (minutes)\n(Departure Date: Wednesday 08:00)", prop={'size':10})
