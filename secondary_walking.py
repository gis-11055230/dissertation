...
for id, pop in worst_pop_points.iterrows():
    ...
    # find the nearest node to the nearest secondary school
    to_node = osm.distance.nearest_nodes(walking_graph, nearest_secondary.x, nearest_secondary.y)
...
worst_pop_points["secondary_walking_astar"] = travel_time_list
...
results = worst_pop_points[["OA21CD", "secondary_walking_astar"]]
...
gm_oas_plot.plot(
    ax = my_ax,						
    column = 'secondary_walking_astar',
    cmap = 'YlOrRd',          
    scheme = 'user_defined',
    ...
    legend_kwds = {
    'loc': 'lower right',
    'title': 'Shortest Walking Time to Nearest Secondary School (mins)',
    'frameon': True,
    'borderpad': 0.6,
    'labelspacing': 0.5
    })
...
legend.set_title("Walking Travel Time to Nearest Secondary School (minutes)", prop={'size':10})
